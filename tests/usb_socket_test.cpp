...
inline bool main_loop(const std::string& dev_path, speed_t speed)
{
	...
	while (true) { // Infinite loop
			...
			while (socket->Status()) { // Reading messages queue
				...
				if (!msg.empty()) {
					if (!socket->Send(&msg, msg.length())) {
						std::cout << "=== Couldn't send data: " << msg << std::endl;
						return false;
					}
		...
		std::cout << "Socket status returned false. Exit\n";
		return false;
	}
}
...
int main() {
	...
	//std::cout << "Enter device path (default /dev/ttyACM0): ";
